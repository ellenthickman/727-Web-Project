...
```
Dropping Canada as outlier for summary stats and plots (Final n = 59), the descriptive statistics for the clustering variables are summarized in Table 6. below.
```{r, echo=FALSE}
library(dplyr)
library(knitr)
library(kableExtra)
...
```
The summary table (Table 7.) presents the mean values of life expectancy, SSR, Gini Index, and GDP per capita for each hierarchical cluster. Across all four clustering variables, we observe considerable variation in country profiles, indicating that the clusters represent distinct "Food System Archetypes" with differing development outcomes. In terms of SSR and GDP per capita, we can define countries as follows:
-   **Cluster 1**: High SSR (globalized exporters) and high GDP per capita. These countries tend to have high life expectancy and low income inequality.
-   **Cluster 2**: Moderate SSR (self-sufficient/localized wheat food chains) and moderate GDP per capita. These countries exhibit moderate life expectancy and income inequality.
-   **Cluster 3**: Low SSR (importers) and low GDP per capita. These countries generally have lower life expectancy and higher income inequality.
...
```{r, echo=FALSE}
# -------------------------------------------------------------
# HIERARCHICAL CLUSTERING PIPELINE FOR:
# life_expectancy, ssr, Gini_Index, gdp, region
# -------------------------------------------------------------
...
# -------------------------------------------------------------
# 4. Cut the tree to define clusters (K = 3 as in k-means)
# -------------------------------------------------------------
...
# -------------------------------------------------------------
# 5. Visualize clusters with PCA plot (prettified)
# -------------------------------------------------------------
fviz_cluster(list(data = df_scaled, cluster = df_clust$hcluster),
             geom = "point",
             ellipse.type = "t",
             palette = "jco",
...
# -------------------------------------------------------------
# 6. Summary table by hierarchical clusters
# -------------------------------------------------------------
df_clust %>%
  group_by(hcluster) %>%
  summarise(across(c(life_expectancy, ssr, Gini_Index, gdp), mean, na.rm = TRUE)) %>%
  kable(col.names = c("Hier. Cluster", "Life Expectancy", "SSR", "Gini Index", "GDP"),
        caption = "Cluster Means by Hierarchical Cluster",
        digits = 2)
```
...
plot(hc, cex = 0.8)         # dendrogram
rect.hclust(hc, k = 3)      # cut K clusters

#add cluster labels
df_clust$cluster_h <- cutree(hc, k = 3)
```
